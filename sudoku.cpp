...
public:
  ...
  bool IsCountZero() const { return count_ == 0; }
...
public:
  int Data(int i, int j) const {return draw_field_[i][j].LastNumber();}
  void MakeOrigin(int i, int j) { draw_field_[i][j].MakeOrigin(); }
  void ChangePosibility(int i, int j, int num) { draw_field_[i][j].ChangePosibility(num); }
  ...
  std::pair<int8_t, int8_t> Predict(){
    for(int i = 0; i<cSheetSize; ++i){
      for(int j = 0; j<cSheetSize; ++j){
        if(not draw_field_[i][j].IsOrigin()){
          while(not draw_field_[i][j].IsLast()){
            draw_field_[i][j].ChangePosibility(draw_field_[i][j].LastNumber());
            draw_field_[i][j].DecreaseCount();
          }
          std::cout<<draw_field_[i][j].LastNumber();
          MakeOrigin(i, j);
          DecreaseCountFree();
          return std::pair<int8_t, int8_t>(i,j);
        }
      }
    }
  }
  std::pair<bool, bool> Filling(std::vector<std::vector<int>> &field) {
    bool updated = false;
    ...
    return std::pair<bool, bool>(updated, is_possible);
  }
};
std::stack<DrawField> call_stack;
std::stack<std::pair<int8_t, int8_t>> coordinations_stack;
...
int main() {
  ...
  while (draw_field.CountFreeCells() != 0) {
    std::pair<bool, bool> out = draw_field.Filling(field);
    if (out.first and out.second) {
      std::cout<<1;
      continue; // recursively place one-possible digit
    }
    else if(not out.second and draw_field.CountFreeCells() != 0){
      std::cout<<2;
      call_stack.top().ChangePosibility(coordinations_stack.top().first, coordinations_stack.top().first, draw_field.Data(coordinations_stack.top().first, coordinations_stack.top().second));
      draw_field = call_stack.top();
      call_stack.pop();
      coordinations_stack.pop();
    }
    else if(not out.first and draw_field.CountFreeCells() != 0){
      std::cout<<3<<"\n";
      call_stack.push(draw_field);
      coordinations_stack.push(draw_field.Predict());
    }
  ...
  Printing(field, is_input, true, true, false); // printing
